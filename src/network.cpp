...
void wifiKeepAlive() {
    ...
    while (wifiMulti.run(WIFI_CONNECTION_TIMEOUT * 1000) != WL_CONNECTED) {
        ...
        if (retries > WIFI_STA_CONNECT_RETRIES) {
            ...
            #ifdef ESP32_DEVICE
            ESP_LOGW("network", "Couldn't connect to any available WiFi network. Fallback to access point mode with ssid '%s'. Please, connect to configure the device. After the configuration a reboot will be necessary.", ssid);
            #endif
            #ifdef ESP8266_DEVICE
